...
class Trainer():
    def __init__(self, args, loader, my_model, my_loss, ckp):
        ...
        self.optimizer = utility.make_optimizer(args, self.scheduler ,self.model)

        if self.args.load != '.':
            self.optimizer.load_state_dict(
                paddle.load(os.path.join(ckp.dir, 'optimizer.pdparams'))
            )
    ...
    def prepare(self, l, volatile=False):
        ...
        def _prepare(tensor):
            if self.args.precision == 'half': tensor = paddle.cast(tensor , 'float16')
            return tensor
